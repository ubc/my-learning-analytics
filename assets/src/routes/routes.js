...
const routes = (courseId, activeViews, activeOnly = true) => {
	const allViews = [
		...
		},
		{
			...
			icon: Grade,
			description:
				'See what assignments have the greatest impact on your grade and set grade goals.',
	...
	return !activeOnly
		? allViews.filter((view) => view.enabled !== undefined)
		: allViews.filter((view) => activeViews[view.viewCode])
}
